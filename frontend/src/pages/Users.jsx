    ...
    <div className="p-8">
      <div className="flex items-end justify-between mb-8">
        <div>
          ...
          <h1 className="font-display font-bold text-4xl tracking-tight">Manage User</h1>
        </div>
        <button
      ...
      <div className="border border-border rounded-xl overflow-hidden" data-testid="users-table">
            ...
            {users.map((u) => (
              <tr key={u.id} className="border-t border-border hover:bg-[#151515] transition-colors" data-testid={`user-row-${u.id}`}>
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-xl ${u.role === "admin" ? "bg-[#00e5ff22] text-primary" : "bg-[#1a1a1a] text-gray-400"}`}>
                    {u.role}
                  </span>
                </td>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    {u.role !== "admin" && (
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-medium border border-border px-2.5 py-1.5 rounded-xl hover:border-primary hover:text-primary transition-colors"
                      >
      ...
      {openNew && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          <div className="bg-[#121212] border border-border rounded-xl w-full max-w-md" data-testid="new-user-dialog">
                ...
                <div className="flex gap-2">
                  {["user", "admin"].map((r) => (
                    <button
                      ...
                      className={`flex-1 py-2 text-sm font-medium rounded-xl border transition-colors ${form.role === r ? "border-primary text-primary bg-[#00e5ff11]" : "border-border text-gray-400 hover:text-white"}`}
                    >
                      {r}
                    </button>
                  ))}
            ...
            <div className="p-6 border-t border-border flex justify-end gap-2">
              <button onClick={() => setOpenNew(false)} data-testid="cancel-user-button" className="text-sm font-medium px-4 py-2 rounded-xl text-gray-400 hover:text-white">Cancel</button>
              <button onClick={create} disabled={saving} data-testid="save-user-button" className="flex items-center gap-1.5 bg-primary text-black font-semibold text-sm px-5 py-2 rounded-xl hover:bg-[#00b3cc] transition-colors disabled:opacity-60">
      ...
      {accessUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          <div className="bg-[#121212] border border-border rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" data-testid="access-dialog">
            <div className="p-6 border-b border-border">
            ...
            <div className="p-6 space-y-3">
              ...
              {storages.map((s) => (
                <div key={s.id} className="flex items-center justify-between border border-border rounded-xl p-3" data-testid={`access-row-${s.id}`}>
                  <div>
                  ...
                  <div className="flex gap-1.5">
                    {["read", "write"].map((p) => (
                      <button
                        ...
                        className={`text-xs font-medium px-3 py-1.5 rounded-xl border transition-colors ${access[s.id] === p ? "border-primary text-primary bg-[#00e5ff11]" : "border-border text-gray-400 hover:text-white"}`}
                      >
                        {p}
                      </button>
                    ))}
            ...
            <div className="p-6 border-t border-border flex justify-end gap-2">
              <button onClick={() => setAccessUser(null)} data-testid="cancel-access-button" className="text-sm font-medium px-4 py-2 rounded-xl text-gray-400 hover:text-white">Cancel</button>
              <button onClick={saveAccess} data-testid="save-access-button" className="bg-primary text-black font-semibold text-sm px-5 py-2 rounded-xl hover:bg-[#00b3cc] transition-colors">Save Access</button>
            </div>
          </div>
        </div>
      )}
